defmodule Dify.Accounts.Scope do
  @moduledoc """
  ...
  caller's membership role in it. Query scoping (`Dify.Repo.scoped/2`) and
  RBAC checks (`Dify.RBAC.can?/3`) both read from this struct, so a function
  that receives a scope needs no other tenancy input.
  """

  alias Dify.Accounts.{Account, Membership, Workspace}
