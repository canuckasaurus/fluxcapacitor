defmodule Dify.Repo do
  ...
  def prepare_query(_operation, query, opts) do
    if opts[:skip_workspace_guard] || not tenant_table?(query) ||
      ...
      raise Dify.Repo.UnscopedQueryError, query: query
    end
  end
...
defmodule Dify.Repo.UnscopedQueryError do
  @moduledoc """
  Raised when a query against a tenant-owned table carries no workspace_id
  filter. Build the query with `Dify.Repo.scoped/2`, add an explicit
  ...
  def message(%{query: query}) do
    ...
    Tenant tables must be queried through Dify.Repo.scoped/2 (or with an
