defmodule DifyWeb.AccountAuthTest do
  use DifyWeb.ConnCase, async: true

  alias Dify.Accounts
  alias Dify.Accounts.Scope
  alias DifyWeb.AccountAuth
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, DifyWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})

    %{
      account: %{account_fixture() | authenticated_at: NaiveDateTime.utc_now(:second)},
      conn: conn
    }
  end
  ...
  describe "logout_account/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      DifyWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "on_mount :require_sudo_mode" do
    ...
    test "redirects when authentication is too old", %{conn: conn, account: account} do
      ...
      assert NaiveDateTime.compare(token_inserted_at, account.authenticated_at) == :gt
      session = conn |> put_session(:account_token, account_token) |> get_session()
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        DifyWeb.Endpoint.subscribe("accounts_sessions:#{Base.url_encode64(token)}")
      end
