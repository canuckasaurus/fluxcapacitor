defmodule FluxWeb.AccountAuthTest do
  ...
  describe "log_in_account/3" do
    ...
    test "redirects to the console when account is already logged in", %{
      conn: conn,
      account: account
    } do
      conn =
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        FluxWeb.Endpoint.subscribe("accounts_sessions:#{Base.url_encode64(token)}")
      end
