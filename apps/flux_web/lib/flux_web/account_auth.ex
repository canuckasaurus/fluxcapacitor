defmodule FluxWeb.AccountAuth do
  @moduledoc """
  Session-based authentication plugs and LiveView `on_mount` hooks for
  console accounts, including remember-me cookies and sudo-mode checks.
  """
  ...
  @remember_me_cookie "_flux_web_account_remember_me"
  @remember_me_options [
    sign: true,
  ...
  def fetch_current_scope_for_account(conn, _opts) do
    with {token, conn} <- ensure_account_token(conn),
      ...
      conn
      |> assign(:current_scope, build_scope(account))
  ...
  # Scopes always carry the account's current workspace and membership so
  # downstream code can rely on Scope.workspace_id/1 and Flux.RBAC checks.
  defp build_scope(nil), do: Scope.for_account(nil)
  defp build_scope(account), do: Accounts.scope_for(account)
  ...
  def disconnect_sessions(tokens) do
    Enum.each(tokens, fn %{token: token} ->
      FluxWeb.Endpoint.broadcast(account_session_topic(token), "disconnect", %{})
    end)
  end
  ...
  the `current_scope`:
      ...
      defmodule FluxWeb.PageLive do
        ...
        on_mount {FluxWeb.AccountAuth, :mount_current_scope}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{FluxWeb.AccountAuth, :require_authenticated}] do
  ...
  def signed_in_path(_), do: ~p"/console"
